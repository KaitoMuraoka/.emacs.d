...
(use-package org
  :ensure nil ; Emacs 組み込みのため MELPA からインストールしない

  :hook
  ;; Org ファイルを開いたとき行の折り返し表示を有効にする
  ;; コードではなく文章を書くため、長い行を画面内で折り返したい
  (org-mode . visual-line-mode)

  :custom
  ;; org ファイルをまとめて置くディレクトリ
  ;; ここに置いたファイルがタスク管理の対象になる
  (org-directory "~/org")

  ;; タスクの状態遷移を定義する
  ;; TODO → DOING → DONE という流れで管理する
  ;; | の右側（CANCELLED）は「完了ではなくキャンセル」を意味する
  (org-todo-keywords
   '((sequence "TODO" "DOING" "|" "DONE" "CANCELLED")))

  ;; DONE になった時刻を自動で記録する
  ;; タスクをいつ完了したかが後から追えるようになる
  (org-log-done 'time)

  ;; 見出しの * を階層に応じてインデントして表示する
  ;; 実際のファイルには * が保存されるが、見た目が整理される
  (org-startup-indented t)

  ;; コードブロック内でシンタックスハイライトを有効にする
  ;; #+begin_src swift ... #+end_src の中が色付きになる
  (org-src-fontify-natively t)

  ;; コードブロックを現在のウィンドウで編集する（別ウィンドウを開かない）
  (org-src-window-setup 'current-window)

  :bind
  ;; グローバルに使えるキーバインド
  ;; org の標準推奨キーバインドに従っている
  ("C-c a" . org-agenda)   ; アジェンダ（タスク一覧）を開く
  ("C-c c" . org-capture)  ; どこからでもタスクやメモを素早く記録
  ("C-c l" . org-store-link)) ; リンクを保存（他のファイルからジャンプできる）
